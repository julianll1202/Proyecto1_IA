...
def breadthFirstSearch(problem: SearchProblem):
    ...
    "*** YOUR CODE HERE ***"
    closed = []
    # Ahora la franja es una cola (FIFO)
    fringe = util.Queue()
    # Agrega el estado inicial a la franja
    fringe.push((problem.getStartState(), []))
    # Mientras haya nodos en la franja
    while not fringe.isEmpty():
...
def uniformCostSearch(problem: SearchProblem):
    ...
    while not fringe.isEmpty():
        ...
        if node not in closed:
            ...
            # Obtenemos los hijos del nodo que vamos a explorar y los agregamos a la franja
            for (newCoordinates, newDirection, newCost) in problem.getSuccessors(node):
                fringe.push((newCoordinates, direction + [newDirection], cost + newCost), cost + newCost)
    util.raiseNotDefined()
    ...
    while not fringe.isEmpty():
        ...
        if node not in closed:
            ...
            for (newCoordinates, newDirection, newCost) in problem.getSuccessors(node):
                ...
                fringe.push((newCoordinates, direction + [newDirection], cost + newCost), totalCost + heuristic(newCoordinates, problem))
    util.raiseNotDefined()
